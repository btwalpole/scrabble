...
function renderSquares(pieces: PieceRecord[]) {
	...
	for (let row = 0; row < 8; row++) {
		for (let col = 0; col < 8; col++) {
			...
			squares.push(
				<div css={squareStyles} style={{ backgroundColor: 'lightgrey' }}>
...
function Piece({ letter }: PieceProps) {
	return <p css={imageStyles}  draggable="false">{letter}</p>; // draggable set to false to prevent dragging of the images
}

export function King() {
	return <Piece letter={'K'} alt="King" />;
}

export function Pawn() {
	return <Piece letter={'P'} alt="Pawn" />;
}
